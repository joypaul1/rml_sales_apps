...
BEGIN
          ...
          ELSIF  P_USER_WANT ='LOST' THEN
            ...
            ELSIF  P_USER_WANT ='FOLLOW_UP'  THEN
                ...
                IF P_DESIGNATION ='BH' THEN
                         Select COUNT(LDS.ID) INTO V_TOTAL_COUNT 
                               from SAL_LEADS_GEN LDS
                                WHERE trunc(LDS.VISIT_DATE)=TRUNC(SYSDATE)
                                AND LDS.ENTRY_BY IN
                                    (SELECT RML_ID
                                        FROM RML_COLL_APPS_USER
                                        WHERE     ACCESS_APP = 'RML_SAL'
                ...
                ELSIF  P_DESIGNATION ='ZH' THEN
                            Select COUNT(ID) INTO V_TOTAL_COUNT 
                                ...
                                AND STATUS IS NULL
                                AND  ENTRY_BY IN   
                                (SELECT DISTINCT RML_ID FROM RML_COLL_APPS_USER WHERE SAL_MM_ZH_ID = P_RML_ID);
            ...
            ELSIF  P_USER_WANT ='FOLLOW_UP_MISS'  THEN
                    IF P_DESIGNATION ='BH' THEN
                         SELECT COUNT(ID) INTO V_TOTAL_COUNT 
                                FROM SAL_LEADS_GEN LDS
                                WHERE trunc(LDS.VISIT_DATE)<TRUNC(SYSDATE)
                                AND TRUNC (LDS.ENTRY_DATE) BETWEEN P_START_DATE AND P_END_DATE
                                AND LDS.STATUS IS NULL
                                AND LDS.ENTRY_BY IN
                                    (SELECT RML_ID
                                        FROM RML_COLL_APPS_USER
